...
export async function initializeUserCourseData(
  username: string,
): Promise<UserCourseData> {
...
export async function getLeaderboard(
  ...
  try {
    ...
    for (const key of keys) {
      ...
      if (data) {
        const coursesCompleted = Object.values(data.courses).filter(
          (c) => c.completedAt,
        ).length;
...
export async function unfollowUser(
  ...
  try {
    ...
    if (followerData) {
      followerData.following = followerData.following.filter(
        (u) => u !== targetUsername,
      );
      await updateUserCourseData(followerUsername, followerData);
    }
    ...
    if (targetData) {
      targetData.followers = targetData.followers.filter(
        (u) => u !== followerUsername,
      );
      await updateUserCourseData(targetUsername, targetData);
    }
