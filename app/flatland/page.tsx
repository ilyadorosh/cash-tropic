...
export default function Flatland() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      ctx.fillText('1D VIEW - What a Flatlander sees (Y compressed into SIZE)', 10, 20);
      ...
      objectsRef.current.forEach(obj => {
        ...
        if (Math.abs(relAngle) < fov / 2) {
          ...
          const screenX = canvas.width/2 + (relAngle / (fov/2)) * (canvas.width/2 - 50);
      ...
      ctx.fillStyle = '#666';
      ctx.font = '12px monospace';
      ctx.fillText('2D MAP - The actual world (you are the red triangle)', 10, map2DY + 15);
      ...
      ctx.lineTo(
        ...
        map2DY + ((player.y + Math.sin(player.angle - fov/2) * fovDist) * map2DHeight / 400)
      );
      ctx.lineTo(
        player.x + Math.cos(player.angle + fov/2) * fovDist,
        map2DY + ((player.y + Math.sin(player.angle + fov/2) * fovDist) * map2DHeight / 400)
      ...
      objectsRef.current.forEach(obj => {
        ...
        if (obj.shape === 'circle') {
          ...
          ctx.fillRect(obj.x - obj.size*0.4, mapY - obj.size*0.4, obj.size*0.8, obj.size*0.8);
        } else {
          ctx.beginPath();
      ...
      ctx.fillText(`Position: X=${player.x.toFixed(0)} Y=${player.y.toFixed(0)}  Angle=${(player.angle * 180 / Math.PI).toFixed(0)}°`, 10, canvas.height - 10);
  ...
  return (
    <div style={{ 
      ...
      <p style={{ color: '#666', marginBottom: 15, textAlign: 'center', maxWidth: 700 }}>
        2D → 1D projection. You exist in 2D but only see a 1D line.<br/>
        <strong>The Y dimension is compressed into the SIZE of objects.</strong><br/>
      ...
      <div style={{ 
        ...
        <p><strong>The insight:</strong> A 2D creature (Flatlander) looking at a 2D world 
        only sees a 1D slice - a line of colors. The second dimension (depth/Y) 
        becomes encoded as <em>apparent size</em>.</p>
        <p>This is exactly how our 3D eyes work - we see a 2D projection of 3D, 
        with depth encoded as size, occlusion, and parallax.</p>
