...
const EXPLAINABLE_CONCEPTS = [
  {
    keyword: "fine structure constant",
    label: "α ≈ 1/137",
    concept:
      "The fine structure constant α = e²/(4πε₀ℏc) ≈ 1/137 - why this specific value?",
  },
  {
    keyword: "szilard",
    label: "Szilard's Engine",
    concept:
      "Szilard's insight: information has thermodynamic cost - E_min = kT ln 2",
  },
  {
    keyword: "entropic gravity",
    label: "Entropic Gravity",
    concept:
      "Entropic gravity: gravity as emergent from information/entropy gradients",
  },
  {
    keyword: "lorentz factor",
    label: "γ Factor",
    concept:
      "The Lorentz factor γ = 1/√(1-v²/c²) and relativistic effects near speed of light",
  },
  {
    keyword: "probability shift",
    label: "EM Fields",
    concept:
      "Electromagnetic fields as probability distribution shifts in discrete spacetime",
  },
  {
    keyword: "discrete spacetime",
    label: "Discrete Spacetime",
    concept:
      "Spacetime as discrete cellular automaton rather than continuous manifold",
  },
  {
    keyword: "landauer",
    label: "Landauer Limit",
    concept:
      "Landauer's principle: minimum energy cost of computation and bit erasure",
  },
  {
    keyword: "speed of light",
    label: "c = 3×10⁸",
    concept:
      "The speed of light as the fundamental ratio of space to time in our universe",
  },
];
...
export default function InteractiveCV({ markdown, latex }: Props) {
    ...
    try {
      const response = await fetch("/api/explain", {
        ...
        body: JSON.stringify({
          concept: concept,
          context:
            "From Illia Dorosh's CV - physics research on entropic gravity, discrete spacetime, and energy systems",
        }),
      });

      const data = await response.json();
      setAiExplanation(
        data.explanation || data.error || "No explanation available",
      );
    } catch (error) {
      console.error("AI explanation error:", error);
      setAiExplanation(
        "Could not connect to AI. Key insight: Szilard showed information has thermodynamic cost - erasing 1 bit costs at least kT ln 2 energy.",
      );
    }
  ...
  const presentConcepts = useMemo(() => {
    ...
    return EXPLAINABLE_CONCEPTS.filter((c) =>
      lower.includes(c.keyword.toLowerCase()),
    );
  }, [markdown]);
  ...
  return (
    <div
      ref={containerRef}
      style={{ display: "flex", gap: 40, flexDirection: "column" }}
    >
      {/* Minimal search bar */}
      <div
        style={{
      ...
      {(aiExplanation || isExplaining) && (
        <div
          style={{
            background:
              "linear-gradient(135deg, rgba(118, 75, 162, 0.15), rgba(0, 255, 170, 0.05))",
      ...
      <div
        style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: 40 }}
      >
        <div
          style={{
        ...
        >
          <div
            ...
            style={{ color: "#e0e0e0", fontSize: "1.05rem", lineHeight: 1.7 }}
          >
            {rendered}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          >
            <div
              style={{ marginBottom: 8, color: "#00ffaa", fontWeight: "bold" }}
            >
              Why this format?
            </div>
      ...
      <style jsx>{`
        ...
        .cv-content :global(hr) {
          border: none;
          border-top: 1px solid rgba(255, 255, 255, 0.1);
          margin: 2rem 0;
        }
        .cv-content :global(.katex) {
          color: #00ffaa;
          font-size: 1.1em;
        }
        .cv-content :global(.katex-display) {
          margin: 1.5rem 0;
          padding: 1rem;
          background: rgba(0, 255, 170, 0.05);
          border-radius: 8px;
