...
export default function ShadowWorld() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (mode === 'both' || mode === 'shadow') {
        ...
        ctx.fillText('You only see X,Y - Z is compressed into SIZE', shadowOffsetX + 10, 35);
        ...
        allObjects.forEach(obj => {
          ...
          ctx.beginPath();
          ctx.arc(shadowOffsetX + shadow.x, shadow.y, shadowSize * 0.3, 0, Math.PI * 2);
          ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(shadowOffsetX + lightShadow.x, lightShadow.y, 8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#fff';
        ctx.font = '10px monospace';
        ctx.fillText('LIGHT', shadowOffsetX + lightShadow.x - 15, lightShadow.y - 12);
      }
      ...
      if (mode === 'both' || mode === 'topdown') {
        ...
        allObjects.forEach(obj => {
          ...
          ctx.beginPath();
          ctx.ellipse(topOffsetX + obj.x, obj.y + 5, obj.size * 0.8, obj.size * 0.3, 0, 0, Math.PI * 2);
          ctx.fill();
          
          // Object
          ctx.fillStyle = obj.color;
          ctx.beginPath();
          ctx.arc(topOffsetX + obj.x, obj.y - zOffset, obj.size * sizeBoost, 0, Math.PI * 2);
          ctx.fill();
          ...
          if (obj.type === 'player') {
            ctx.fillStyle = '#fff';
            ctx.font = '10px monospace';
            ctx.fillText(`Z: ${obj.z.toFixed(0)}`, topOffsetX + obj.x + 15, obj.y - zOffset);
          }
        });
        ...
        ctx.fillStyle = '#ffff00';
        ctx.beginPath();
        ctx.arc(topOffsetX + light.x, light.y - lightZOffset, 10, 0, Math.PI * 2);
        ctx.fill();
      ...
      ctx.fillText(`Player: X=${player.x.toFixed(0)} Y=${player.y.toFixed(0)} Z=${player.z.toFixed(0)}`, 10, canvas.height - 40);
      ctx.fillStyle = '#888';
      ctx.font = '12px monospace';
      ctx.fillText('WASD=Move(X,Y)  Q/E=Up/Down(Z)  1/2/3=View Mode', 10, canvas.height - 20);
  ...
  return (
    <div style={{ 
      ...
      <p style={{ color: '#666', marginBottom: 20, textAlign: 'center', maxWidth: 600 }}>
        3D → 2D projection. You exist in 3D but only see 2D shadows.<br/>
        Move in Z (Q/E) and watch how shadows change size and position.<br/>
        <strong>Projection compresses one dimension into another property (size).</strong>
      </p>
      ...
      <div style={{ 
        ...
        <p><strong>The insight:</strong> When you project 3D→2D, the Z dimension doesn't disappear - 
        it becomes encoded in other properties (shadow size, blur, position offset).</p>
