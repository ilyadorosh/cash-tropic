...
export default async function LeaderboardPage() {
  ...
  const users: LeaderboardUser[] = await Promise.all(
    keys.map(async (key) => {
      ...
      const coursesEnrolled = courseData
        ? Object.keys(courseData.courses).length
        : 0;
  ...
  const activeUsers = users.filter(
    (u) => u.totalPoints > 0 || u.coursesEnrolled > 0,
  );
          ...
          {topUsers.length === 0 ? (
            <div style={{ padding: "40px", textAlign: "center", opacity: 0.6 }}>
              ...
              <p style={{ fontSize: "0.9rem" }}>
                Sei der Erste! Starte einen Kurs und sammle Punkte.
              </p>
            </div>
          ) : (
            topUsers.map((user) => (
